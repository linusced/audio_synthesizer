#include "../../header/custom_gui/waveform.hpp"

customGUI::Waveform::Waveform(unsigned int textureWidth, unsigned int textureHeight, gui::Constraints *constraints)
...
customGUI::Waveform::~Waveform()
{
    if (texture)
        delete texture;
}

void customGUI::Waveform::update(audio::Waveform const *waveData)
{
    for (size_t i = 0; i < textureBuffer.size(); i += 3)
        textureBuffer[i] = textureBuffer[i + 1] = textureBuffer[i + 2] = 0;

    waveData->draw(textureBuffer, textureWidth, textureHeight, 3);
...
void customGUI::Waveform::resizePoly(int &windowWidth, int &windowHeight)
{
